...
def test_notebook_with_security(system_mocked, sys_mocked):
    ...
    system_mocked.assert_called_once_with("SPARK_CONF_DIR=/opt/spark/conf YARN_CONF_DIR=/opt/spark/conf jupyter notebook --notebook-dir /tmp/notebooks --ip 0.0.0.0 --port 8888 --no-browser --config ".fornat(os.path.join(os.environ["SPARK_HOME"], "conf")) +
...
def test_jupyter_lab(system_mocked, sys_mocked):
    ...
    system_mocked.assert_called_once_with(
        "SPARK_CONF_DIR={0} YARN_CONF_DIR={0} jupyter-lab --notebook-dir /tmp/notebooks --ip 0.0.0.0 --port 8888 --no-browser --NotebookApp.token=".format(os.path.join(os.environ["SPARK_HOME"], "conf")))
...
def test_jupyter_lab_with_security(system_mocked, sys_mocked):
    ...
    system_mocked.assert_called_once_with(
        "SPARK_CONF_DIR={0} YARN_CONF_DIR={0} jupyter-lab --notebook-dir /tmp/notebooks --ip 0.0.0.0 --port 8888 --no-browser".format(os.path.join(os.environ["SPARK_HOME"], "conf")))
